...
class DecoderGraphBuilder(GraphBuilder):
    ...
    def inputs(
        ...
        if cache_2d:
            ...
            cache_ids = s32[position_sizes].Parameter(
                parameter_number=1
            )  # 2d cache_ids
        else:
            cache_ids = s32[n_active_tokens].Parameter(
                parameter_number=1
            )  # 1d cache_ids
...
class DecoderGraph(NeuronBaseSerializer):
    ...
    def to_neuron(self):
        ...
        divisor = int(
            os.environ.get(
                "NEURON_VOCAB_PAD_DIVISOR", str(self.neuron_config.tp_degree)
            )
        )
...
class NeuronHloDecoderModel(NeuronModelBase):
    def __init__(self, config, neuron_config, cpu_model, hlo_builder):
        ...
        self.decoder_lm_head = DecoderGraph.init_token_decoder(
            config, neuron_config, hlo_builder
        )
