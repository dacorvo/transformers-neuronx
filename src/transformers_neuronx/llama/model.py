...
class LlamaForSampling(NeuronModelBase):
    ...
    def load_weights(self):
        ...
        # Adding core_id for seq-norm (vocab parallel is used as default with seq-par norm)
        add_core_id = (
        ...
        if add_core_id:
            self.decoder_lm_head.add_pre_layer_parameter(
                torch.arange(self.config.tp_degree), sharding=0
            )
    ...
    def preprocess_and_embed(self, input_ids, cache_ids=None, start_ids=None, **kwargs):
        padded_inputs, *rst = self._preprocess(
            input_ids, start_ids=start_ids, cache_ids=cache_ids, **kwargs
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if last_token_id is not None:  # preprocess_and_embed() has already been invoked
            ...
            input_ids, input_embeddings, *rst = self.preprocess_and_embed(
        ...
        return self._postprocess(
            original_input_ids, logits, start_ids=start_ids, **kwargs
        )
