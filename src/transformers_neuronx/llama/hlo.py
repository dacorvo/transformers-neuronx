...
class LlamaForSamplingNoEmbeddingHlo:
    ...
    def native_kernel_layer(
        ...
        def _mlp_fused_add_kernel(
            attn_output, hidden, ln_w, gate_w, up_w, down_w, out, fused_rmsnorm=True
        ):
            mlp_fused_add_isa_kernel(
                attn_output,
                hidden,
        ...
        if enable_qkv_kernel:
            ...
            if len(fused_out) == 3:
                ...
                attn_output, out_attn_k_cache, out_attn_v_cache, fused_added_hidden = (
                    fused_out
                )
        else:
            ln_hidden = (
    ...
    def fused_rmsnorm_qkv(
        ...
        if fused_add:
            nki_output = nki_call(
                ...
                pre_attn_ln_weight,
                output_HloShapes=[
                    hidden.dtype[n_seqs, n_active_tokens, hidden_size_tp]
                ],
            )
        else:
            nki_output = nki_call(
    ...
    def attention(
        ...
        if active_mask is not None:
            ...
            if (
                ...
                and self.neuron_config.continuous_batching
            ):
        ...
        output = attention.output(
            context, out_weight, out_bias, tp_degree, self.neuron_config
        )
