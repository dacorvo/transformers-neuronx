...
def hlo_rotary_embedding(
    ...
    sinusoid_inp = functional.dot_general(
        cache_ids, inv_freq, dimension_numbers=dot_dims
    )
...
def rotate_vec(q, sin_r, cos_r, rotary_percentage=1):
    ...
    if rotary_percentage == 1:
        ...
        q_rotary_rot_up = functional.ax_minus_by(
            cos_r, q_rotary_up, sin_r, q_rotary_down
        )
        q_rotary_rot_down = functional.ax_plus_by(
            cos_r, q_rotary_down, sin_r, q_rotary_up
        )
        q_rotary_rot = functional.concatenate(
